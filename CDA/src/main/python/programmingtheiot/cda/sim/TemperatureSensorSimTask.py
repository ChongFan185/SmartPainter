...
class TemperatureSensorSimTask(BaseSensorSimTask):
	...
	def __init__(self, dataSet=None):
		super(TemperatureSensorSimTask, self).__init__(SensorData.TEMP_SENSOR_TYPE, dataSet=dataSet,
		                                               minVal=SensorDataGenerator.LOW_NORMAL_INDOOR_TEMP,
		                                               maxVal=SensorDataGenerator.HI_NORMAL_INDOOR_TEMP)

	def generateTelemetry(self) -> SensorData:
		# call super class method
		sd = super(TemperatureSensorSimTask, self).generateTelemetry()
		sd.setName(ConfigConst.TEMP_SENSOR_NAME)
		return sd
